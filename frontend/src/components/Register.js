...
function Register({display,style}){
    ...
    return(
        <div className="col-span-3 relative h-screen flex flex-col justify-center gap-12 text-xl items-center" style={style}>
            ...
            <h1 className="text-white relative text-center font-extrabold text-5xl pr-15 tracking-widest">register</h1>
            <form className="flex flex-col gap-8 z-20 relative font-noto text-[1rem] items-start" onSubmit={(e)=>{e.preventDefault();handleRegister(email,password)}}>
                ...
                <div className="w-3/4 relative flex items-center">
                    ...
                    <input className="rounded-sm p-2 pl-7 text-slate-400 outline-none bg-transparent border-b border-yellow-300" placeholder="password" name={password} value={password} onChange={(e)=>{setPassword(e.target.value)}}/>
                </div>

                <div className="w-3/4 relative flex items-center">
                    <i className="fa fa-key absolute left-0 text-white" aria-hidden="true"></i>
                    <input className="rounded-sm p-2 pl-7 text-slate-400 outline-none bg-transparent border-b border-yellow-300" placeholder="confirm password" value={Cpassword} onChange={(e)=>{setCpassword(e.target.value)}}/>
