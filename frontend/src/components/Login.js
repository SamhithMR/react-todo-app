...
function Login({redirects,display,style}){
    ...
    return(
        <div className="col-span-5 md:col-span-3 relative h-screen flex flex-col justify-center gap-12 text-xl items-center " style={style}>
            ...
            <h1 className=" text-white relative text-center font-bold md:font-extrabold text-5xl pr-15 tracking-widest">Welcome back</h1>
            <form className="flex flex-col gap-8 z-20 relative font-noto text-[1.1rem] items-start" onSubmit={(e)=>{e.preventDefault();handleLogin(email,password)}}>
                ...
                <i className="fa fa-key absolute left-0 text-white" aria-hidden="true"></i>
                    <input className="text-gray-100 rounded-sm p-2 pl-7 outline-none bg-transparent border-b border-yellow-300" placeholder="password" name={password} value={password} onChange={(e)=>{setPassword(e.target.value)}}/>
