...
function Login({redirects,display, style}){
    ...
    return(
        <div className="col-span-3 relative h-screen flex flex-col justify-center gap-12 text-xl items-center" style={style}>
            ...
            <form className="flex flex-col gap-8 z-20 relative  items-start" onSubmit={(e)=>{e.preventDefault();handleLogin(email,password)}}>
                {/* <h3 className="text-xl text-center text-white pl-2 text-2xl tracking-widest font-bold">Login</h3> */}
            ...
            <p className="z-20 relative">don't have an account <button className="text-white" onClick={()=>{display() ;setEmail("");setPassword("");setErr("");}}>register</button></p>
        </div>
